...
public class CLI {
    ...
    @Option(name = "-simulatedAnnealing", usage = "Simulated annealing algorithm. Supported are: LINEAR and EXPONENTIAL")
    private String SIMULATED_ANNEALING = "LINEAR";
    private SimulatedAnnealing simulatedAnnealing = SimulatedAnnealing.LINEAR;

    @Option(name = "-restartSA", usage = "Number of rounds when SA should be restarted")
    private Integer restartSA = null;
    ...
    public Config parseArgs(String[] args) throws FileNotFoundException {
        ...
        try {
            ...
            if (SIMULATED_ANNEALING.compareToIgnoreCase(SimulatedAnnealing.LINEAR.toString()) == 0) {
                simulatedAnnealing = SimulatedAnnealing.LINEAR;
            } else if (SIMULATED_ANNEALING.compareToIgnoreCase(SimulatedAnnealing.EXPONENTIAL.toString()) == 0) {
                simulatedAnnealing = SimulatedAnnealing.EXPONENTIAL;
            } else {
                throw new IllegalArgumentException("Simulated annealing selection policy is not supported");
            }
        ...
        return new Config().setRandNeighborsSampleSize(randNeighborsSampleSize)
                ...
                .setAlpha(ALPHA)
                .setSimulatedAnnealing(simulatedAnnealing)
                .setRestartSA(restartSA);
    }
}
